package de.envite;

import org.springframework.stereotype.Component;

@Component
public class JobWorker {

    @io.camunda.client.annotation.JobWorker(type = "testing1")
    public void dummyWorker1() {
//        LOG.info("dummyWorker1  ---  Done");
//        System.out.println("Testing 1 erledigt");
    }

    @io.camunda.client.annotation.JobWorker(type = "testing2")
    public void dummyWorker2() {
//        LOG.info("dummyWorker2  ---  Done");
//        System.out.println("Testing 2 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing3")
    public void dummyWorker3() {
//        LOG.info("dummyWorker3  ---  Done");
//        System.out.println("Testing 3 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing4")
    public void dummyWorker4() {
//        LOG.info("dummyWorker4  ---  Done");
//        System.out.println("Testing 4 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing5")
    public void dummyWorker5() {
//        LOG.info("dummyWorker5  ---  Done");
//        System.out.println("Testing 5 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing6")
    public void dummyWorker6() {
//        LOG.info("dummyWorker6  ---  Done");
//        System.out.println("Testing 6 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing7")
    public void dummyWorker7() {
//        LOG.info("dummyWorker7  ---  Done");
//        System.out.println("Testing 7 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing8")
    public void dummyWorker8() {
//        LOG.info("dummyWorker8 ---  Done");
//        System.out.println("Testing 8 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing9")
    public void dummyWorker9() {
//        LOG.info("dummyWorker9  ---  Done");
//        System.out.println("Testing 9 erledigt");

    }

    @io.camunda.client.annotation.JobWorker(type = "testing10")
    public void dummyWorker10() {
//        LOG.info("dummyWorker10  ---  Done");
//        System.out.println("Testing 10 erledigt");
    }
//    @JobWorker(type = "testing11")
//    public void dummyWorker11() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing12")
//    public void dummyWorker12() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing13")
//    public void dummyWorker13() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing14")
//    public void dummyWorker14() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing15")
//    public void dummyWorker15() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing16")
//    public void dummyWorker16() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing17")
//    public void dummyWorker17() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing18")
//    public void dummyWorker18() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing19")
//    public void dummyWorker19() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing20")
//    public void dummyWorker20() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing21")
//    public void dummyWorker21() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing22")
//    public void dummyWorker22() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing23")
//    public void dummyWorker23() {
////        LOG.info("dummyWorker10  ---  Done");
//    }
//    @JobWorker(type = "testing24")
//    public void dummyWorker24() {
////        LOG.info("dummyWorker10  ---  Done");
//    }

}


